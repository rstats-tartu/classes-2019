...
med %>% 
  ggplot() +
  geom_line(mapping = aes(x = date, y = adj_price, colour = area)) +
  facet_wrap(~ county)
```

As we can see, based on real values, there was still some potential for growth in 2017. But what's the glich at the very end of the lines? We try to find that out using arrange..
...
ggplot(data = transactions) +
  geom_histogram(aes(price_max), bins = 30)
```

We can have better look at the distibution by using log transformaton.
```{r}
pricelog <- mutate(transactions, price_max = log10(price_max))
ggplot(data = pricelog) +
  geom_histogram(aes(price_max), bins = 30) +
  xlab("Maximum price, log10")
```
...
```{r}
summarise(transactions, mean_volume_per_month = mean(price_total, na.rm = TRUE))
```
...
summarise(by_county, mean_volume_per_month = mean(price_total, na.rm = TRUE))
```
...
ggplot(data = volume) +
  ...
  coord_flip() +
  labs(title = "Volume of transactions per month\nwith 70-245 m2 apartments",
       x = element_blank(),
       y = "Total price per month, euros")
```

There are three steps to prepare this data:

- Group ransactions by county and area.
