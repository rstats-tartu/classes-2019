...
3. The name of the variable whose values are spread over the cells. I call that value, and here it's the average bruto wage (Palk).
...
Let's merge wage data with average price per m^2 feom transactions data:
```{r}
...
(price_permonth <- trans %>% 
  group_by(year, month) %>% 
  summarise(av_price = mean(price_unit_area_mean, na.rm = TRUE)))
```

We need to change months to integers:
```{r}
wage_tidy <- mutate(wage_tidy, month = case_when(
...
salary_and_prices %>% 
  ggplot() +
  geom_point(aes(x = wage, y = av_price, color = factor(year)))
```
