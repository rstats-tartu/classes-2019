...
Here we get started with R from the standpoint of visualization.
...
ggplot(data = mpg) + 
  ...
  facet_wrap(~ displ)
```
Each raw value is contverted to categorical value and gets its own facet?
...
4. Read ?facet_wrap. What does nrow do? What does ncol do? What other options control the layout of the individual panels? Why doesn't facet_grid() have nrow and ncol argumentt.
