...
Which one from these two is more explicit? Note that comma here in filter function between logical evaluations means "&".
...
regular expressions are covered in *tidyverse* by the **stringr** package. The stringr function that is useful within filter is str_detect(). Although, stringr belongs to tidyverse, it is not loaded together with core tidyverse packages by default, so you need to load it separately.


```{r}
library(stringr)
str_detect(c("banana","foo bar", "foo-bar", "foobar"), "foo.?bar")
```
...
2. Sort viruses to find largest viruses:
...
Another way, exclude as a vector:
```{r}
...
3. Select all variables from 'diabetes' dataset that contain string 'BP' (note case!). 

```{r}
diabetes <- read_delim(here("data", "diabetes.csv"), delim = ";")
diabetes
```
...
```{r}
?select
diabetes_bp <- select(diabetes, contains("BP", ignore.case = FALSE))
diabetes_bp
```
