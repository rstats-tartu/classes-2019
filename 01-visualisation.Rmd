...
```{r}
diameters <- data_frame(x = 1:4, y = 1, diameter = c("tiny", "small", "big", "very big"))
diameters
```
...
2. Update code. Map *size* aesthetic to class:
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy))
```
...
```{r}
# try HEX code "#0000ff" or rgb(0,0,1)
```
...
- use fill = "green", color = "blue", to change the appearance of the points, and
- adjust size of the points (size = 3) for better visibility. 
