...
Reducing duplication is achieved by **use of functions**, either generic (mean etc) or custom, and **by iteration** (repetition) of the same operation on different columns or datasets/-splits.
...
But tidyverse has **purrr** package that contains map() family functions to loop over vectors and lists. 


There is different function for each type of input:
...
Previous code could be replaced by dplyr summarise functions, at least when input is a dataframe.
...
map_dbl(x, mean) # map_dbl returns vector
map(x, mean) # returns list
```
